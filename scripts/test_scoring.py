...
Verifica:
  ...
  2. calc_context_stars — estrellas de contexto
  3. determine_final_signal — señal final
  4. Integración: casos reales conocidos
"""
import sys
import os
...
s = calc_score(rsi=52, ema20=110, ema50=100, sma200=90, price=115,
...
s = calc_score(rsi=78, ema20=90, ema50=110, sma200=120, price=88,
               vol_ratio=40, mansfield_rs=-4.0, momentum_4w=-12.0, recent_high=115)
check_range("Setup bajista → score bajo", s['score'], 0, 30)

# EMA bajista → siempre penaliza
s = calc_score(rsi=50, ema20=95, ema50=100, sma200=None, price=96,
               vol_ratio=100, mansfield_rs=0.5, momentum_4w=2.0, recent_high=105)
check("EMA bajista → ema_trend = -10", s['breakdown']['ema_trend'], -10)

# RSI sobrecompra extrema → penaliza
s = calc_score(rsi=80, ema20=110, ema50=100, sma200=90, price=112,
...
s = calc_score(rsi=25, ema20=110, ema50=100, sma200=90, price=112,
               vol_ratio=100, mansfield_rs=1.0, momentum_4w=5.0, recent_high=120)
check("RSI 25 → rsi = +5", s['breakdown']['rsi'], +5)

# Volumen muy bajo → penaliza
s = calc_score(rsi=50, ema20=110, ema50=100, sma200=90, price=112,
...
# SMA200 recuperación: precio bajo SMA pero EMA alcista y momentum positivo → -3 no -6
s = calc_score(rsi=50, ema20=110, ema50=100, sma200=120, price=112,
...
s = calc_score(rsi=85, ema20=80, ema50=100, sma200=120, price=78,
...
c = calc_context_stars(score=70, ema20=110, ema50=100, mansfield_rs=2.0,
...
# Mansfield < -2 con técnica alcista → baja 1 estrella
c = calc_context_stars(score=70, ema20=110, ema50=100, mansfield_rs=-3.0,
                       next_earnings=None, ex_dividend_date=None,
                       fundamentals=f_clean, price=100.0, max_days=20)
check("Mansfield -3 + alcista → 2 estrellas (3-1)", c['stars'], 2)

# Score < 45 → 0 estrellas (no aplica)
c = calc_context_stars(score=40, ema20=110, ema50=100, mansfield_rs=2.0,
                       next_earnings=None, ex_dividend_date=None,
                       fundamentals=f_clean, price=100.0, max_days=20)
