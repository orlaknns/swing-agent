    ...
    batchPollRef.current = setInterval(async () => {
      try {
        ...
        Object.entries(job.results || {}).forEach(([ticker, data]) => {
          ...
          console.log('[batch-poll] cacheAnalysis:', ticker, !!data.error, data.error || '', !data.error ? JSON.stringify(data).slice(0,200) : '')
        ...
        <div style={{ display: tab === 'watchlist' ? 'block' : 'none' }}>
          <div style={{ maxWidth:960, margin:'0 auto', padding:'0 20px' }}>
            ...
            {(() => {
              ...
              const stale = activeWatchlist.filter(t => {
                const s = analysisCache[t]?._savedAt
                if (!s) return true
              ...
              return (
                <div style={{ marginBottom:10, display:'flex', alignItems:'center', gap:8,
                  ...
                  <button onClick={() => runQueueSwing(stale)}
          ...
          <div style={{ maxWidth:960, margin:'0 auto', padding:'0 20px' }}>
            {!isLoaded ? (
              ...
              <>
                ...
                {(() => {
                  if (queue.length > 0) return null
                  const stale = monitorWatchlist.filter(t => {
                    const s = analysisCache[t]?._savedAt
                    if (!s) return true
                    return (Date.now() - new Date(s)) > 24 * 60 * 60 * 1000
                  })
